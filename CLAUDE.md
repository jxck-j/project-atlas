...
below); Diplomacy/Technology/Current Status remain "Awaiting data feed" — their sourcing/weighting isn't
locked yet (design doc §9). `IntelRow`'s bar fill is a single solid color, not a gradient, computed once per row by
...
different row data. Diplomacy/Technology/Current Status still have no component data to drill into.
...
(see below). The remaining three thumbnails render the identical "Awaiting data feed — no assessment data
