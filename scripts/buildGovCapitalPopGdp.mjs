...
//
//   - World Bank API v2 (population, GDP, land area) — no key, REST/JSON.
//     Queried over a range (WORLD_BANK_LOOKBACK_START_YEAR..
//     WORLD_BANK_PRIMARY_YEAR) rather than a single year, and the most
//     recent year with a non-null value in that range is used — but the
//     actual year used is always recorded (see gdpYear/populationYear/
//     areaYear below) and, whenever it isn't WORLD_BANK_PRIMARY_YEAR, logged
//     as a gap. A country with genuinely no value anywhere in the range is
//     left unscored, not backfilled with a fabricated number.
...
const AREA_INDICATOR = 'AG.LND.TOTL.K2'
...
// countries x ~4 requests each (factbook + 3 World Bank indicators) is
// ~770 requests; unbounded concurrency against two public, unauthenticated
...
async function buildProfile(country) {
  ...
  const [gdp, population, area] = alpha3
    ? await Promise.all([
        ...
        resolveWorldBankIndicator(country, alpha3, AREA_INDICATOR, 'area'),
      ])
    : (logGap(country.name, 'population + gdp + area', 'No ISO alpha-3 code resolved — left unscored.'),
       [{ value: undefined, year: undefined }, { value: undefined, year: undefined }, { value: undefined, year: undefined }])
  ...
  return {
    ...
    gdpYear: gdp.year,
    areaKm2Raw: area.value == null ? undefined : Math.round(area.value),
    areaYear: area.year,
...
function economicsEntryToTs(p) {
  ...
  if (p.areaKm2Raw != null) fields.push(`areaKm2: ${p.areaKm2Raw}`)
  if (p.areaYear != null) fields.push(`areaYear: ${p.areaYear}`)
...
console.log(`Wrote ${ECONOMICS_OUTPUT}: ${profiles.filter((p) => p.populationRaw != null || p.gdpRaw != null || p.areaKm2Raw != null).length} countries with at least one figure.`)
