...
src/
  ...
  hud/                       Plain DOM/Tailwind overlay, siblings of the Canvas
    ...
    NewsPanel.tsx                (v6.11.0) Full-screen view behind TopNav's NEWS tab
                                 — featured row plus a thumbnail tile grid, sticky
                                 search bar, infinite-scroll pagination, region-hub
                                 pills and organization/person/asset chips that
                                 filter the grid, and a JUST IN badge on items
                                 under an hour old
    newsFilterStore.ts           (v6.11.0) Carries one country id across the tab
                                 switch from IntelligencePanel.tsx's Recent News
                                 "MORE" link to NewsPanel.tsx — consumed once on
                                 open, not a persistent filter
    newsSeverityStyles.ts        (v6.11.0) Severity-tier color/label, shared by
                                 NewsPanel.tsx and IntelligencePanel.tsx's Recent
                                 News section so the two can't drift apart
  ...
  data/
    ...
    newsTypes.ts                (v6.11.0) NewsItem schema, topic tags, and severity
                                tiers. Design: news-engine-design.md (what v1 was
                                built against); news-sourcing-design.md is the v2
                                redesign (Event/dossier model), not built
    registry/NewsRegistry.ts    (v6.11.0) registerNewsItem/getNewsItem/
                                getNewsItems/getNewsItemsByEntityId/
                                getNewsItemsByTag — same non-reactive Map pattern
                                as CountryRegistry.ts/GeoEntityRegistry.ts
    useNewsFeatures.ts          (v6.11.0) Fetch-once store for public/data/
                                news.json (see below), with hooks for all/
                                published/pending items. Raw JSON rather than a
                                generated .ts const, so a feed can refresh
                                without an app rebuild
    countryRegions.ts           (v6.11.0) Generated Country.region data (World
                                Bank's 7 regions) keyed by numeric ISO id — feeds
                                NewsPanel.tsx's region hubs. Generated by
                                buildCountryRegions.mjs, don't hand-edit
...
scripts/
  ...
  generateClaimsDoc.mjs        (v3.1.1, rewritten v3.1.3, npm run
                             ...
                             do under tsx)
  buildNews.mjs              (v6.11.0, npm run build:news) News pipeline: 15
                             public RSS feeds -> severity-tiered, corroboration-
                             gated, duplicate-merged NewsItems ->
                             public/data/news.json. Runs at build time, never in the
                             browser
  buildCountryRegions.mjs    (v6.11.0, npm run build:regions) World Bank country
                             classification -> src/data/countryRegions.ts
public/geo/
  ...
  us-cities/{state}.json         (v4.2) 56 per-state geometry shards — one
                             ...
                             useUsCityOutline.ts
public/data/
  news.json                  (v6.11.0) Generated output of buildNews.mjs —
                             fetched at runtime by useNewsFeatures.ts
```
