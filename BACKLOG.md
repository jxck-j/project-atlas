...
**Generated by `npm run build:profiles` (`scripts/buildGovCapitalPopGdp.mjs`), 2026-08-13.** Every field below couldn't be sourced cleanly from World Bank/factbook.json this run: either it fell back to countryProfiles.ts's prior value (capital/government), cited an older year than 2024 explicitly (population/GDP — see countryEconomics.ts's populationYear/gdpYear), or was left unscored entirely (a genuine gap in the source, no figure at any year in the lookback window). Re-running the script regenerates this list — don't hand-edit it.
