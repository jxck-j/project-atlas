...
**Generated by `npm run build:economy` (`scripts/buildEconomy.mjs`), 2026-08-21.** Economy category component fields that couldn't be sourced cleanly this run — left unscored, not guessed. Re-running the script regenerates this list.
...
- **[Cuba] GDP PPP:** World Bank has no NY.GDP.MKTP.PP.CD value for CUB in range — left unscored.
...
- **[Liechtenstein] GDP PPP:** World Bank has no NY.GDP.MKTP.PP.CD value for LIE in range — left unscored.
...
- **[Monaco] GDP PPP:** World Bank has no NY.GDP.MKTP.PP.CD value for MCO in range — left unscored.
...
- **[North Korea] GDP PPP:** World Bank has no NY.GDP.MKTP.PP.CD value for PRK in range — left unscored.
...
- **[South Sudan] GDP PPP:** World Bank has no NY.GDP.MKTP.PP.CD value for SDS in range — left unscored.
...
**Generated by `npm run build:military` (`scripts/buildMilitary.mjs`), 2026-08-20.** Military category coverage-gap fields (expenditure/%GDP/personnel) that couldn't be sourced cleanly this run — left unscored, not guessed. Re-running the script regenerates this list.
...
- **Intelligence Engine** — as of v6.3.1, MILITARY is wired to real data
  ...
  `CLAUDE.md`'s v6.6.0 entry). DIPLOMACY / TECHNOLOGY / CURRENT STATUS are
  still hardcoded "Awaiting data feed" placeholders in both surfaces, for
  every entity, country or GeoEntity alike. Deliberately left unfabricated
  — see `README.md`'s "Notes for future work": fabricating country-level
  assessments for a defense-context demo isn't something to do casually.
  Diplomacy/Technology weights aren't locked yet — design doc §9. Whichever
  of these three gets real data next will need a `buildXRows`/`X_COLUMNS`
  pair added to `AnalyticsPanel.tsx` and an `XDrilldown` added to
  `IntelligencePanel.tsx`, following the Military/Economy pattern.
