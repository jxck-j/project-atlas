...
- 39 Event(s) published from 1187 article(s) across 23/24 feeds; 0 pending confirmation.
- 425 article(s) sat in an Event below its corroboration floor (a lone report never publishes — every tier needs 2+ distinct sources) and were not emitted.
- 1 feed(s) failed to fetch this run:
  - france-24 (https://www.france24.com/en/rss): 503 Service Unavailable
- 101 article(s) dropped for no resolvable country link (person names like "Trump"/"Merz" and orgs like the ICC don't resolve — see the standing item above) (first 25):
  - "Bondi Beach Terror Suspects Condemned &#x2018;Zionists&#x2019; and Amassed an Arsenal" (wsj-news)
  - "EU wants drones to spray pesticides, but it could backfire" (deutsche-welle)
  - "Washington&#x2019;s Deadly New Headache: an al Qaeda-Houthi Terrorist Alliance" (wsj-news)
  - "Bank of England must raise interest rates, says chief economist" (telegraph)
  - "Kosovo Parliament to Reconvene, ‘Chaos’ Looms over Election of President" (balkan-insight)
...
`scripts/buildNewsEvents.mjs`) and Phase 3 (LLM classification/grouping, `--llm`, not yet run live) are built; everything below is still open. (The `buildNews.mjs` gap-report block that follows is v1's generated output, a separate list.)
