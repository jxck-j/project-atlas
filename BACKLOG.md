...
- 15 Event(s) published from 1170 article(s) across 24/24 feeds; 0 pending confirmation.
- 439 article(s) sat in an Event below its corroboration floor (a lone report never publishes — every tier needs 2+ distinct sources) and were not emitted.
- 401 article(s) dropped for no resolvable country link (person names like "Trump"/"Merz" and orgs like the ICC don't resolve — see the standing item above) (first 25):
  - "Imran Khan’s family says three sisters detained ahead of Islamabad march" (al-jazeera)
  - "Trump administration readies sweeping sanctions against ICC, reports say" (al-jazeera)
  - "The Sahel and the UN: a smaller footprint, a different role" (al-jazeera)
  - "Man Utd salvage Fulham draw as pressure mounts on Carrick" (al-jazeera)
  - "Atletico Madrid beat 10-man Real Madrid 2-1 in feisty La Liga derby" (al-jazeera)
  - "Converse pulls 'deeply upsetting' advert after backlash" (bbc)
  - "Google's Gemini AI hacked three companies in security test" (bbc)
  - "Journalists denied White House access after Trump banned some media outlets" (bbc)
  - "Cultural treasures are being destroyed worldwide - so why is no-one being punished?" (bbc)
  - "Our head teacher was an abuser. We joined forces to get justice - now we're married" (bbc)
  - "Trump escalates media fight after week of setbacks" (bbc)
  - "Not all AI workers think the tech could kill everyone" (bbc)
  - "Watch: Fans react as Ed Sheeran speaks out over Macklemore controversy at show" (bbc)
  - "Watch: Concertgoers and pro-Palestinian activists arrive at Ed Sheeran concert" (bbc)
  - "'Enough is enough': Women protest after string of murders near Johannesburg" (bbc)
  - "Is the Kennedy Center losing 'hundreds of millions of dollars'?" (bbc)
  - "Torres strikes again as PSG beat bitter rivals Marseille 2-1" (france-24)
  - "The Deportation Project: Inside the Trump administration's secret deals to expel people worldwide" (france-24)
  - "Exit polls show 'disaster' for Merz' CDU party in two state elections" (france-24)
  - "Louisiana firefighters find feline native to sub-Saharan Africa while responding to house fire" (guardian)
  - "Man, 82, dies after brawl with 76-year-old over sunlounger on Greek beach" (guardian)
  - "Lib Dems propose big tech levy to fund £250m online crime agency" (guardian)
  - "Specialist courtrooms to prioritise rape cases in England and Wales" (guardian)
  - "Deaths of three women in sea at Brighton had no criminal involvement, police say" (guardian)
  - "Patrick Clancy, ex-husband of Lindsay Clancy, opens up on children’s killings in first televised interview" (guardian)
...
(`src/news/` schema, pure logic, `sources.json`/`systemicThemes.json`) and Phase 2 (the Event build pipeline,
`scripts/buildNewsEvents.mjs`) are built; everything below is still open. (The `buildNews.mjs` gap-report block that follows is v1's generated output, a separate list.)
...
- **Event clustering is a headline-only heuristic, and the 4-outlet Critical fallback barely fires because of it**
  (Phase 2, `src/news/clustering.ts`). Verified on one live pull (2026-09-20): a genuine Critical story (Houthi
  attack on Riyadh, 6+ outlets) splits into a 3-outlet and a 2-outlet Event because the outlets' headlines share
  too few words, so neither reaches Critical's four. Loosening it is unsafe — the same pull showed an earlier,
  looser version fusing three unrelated "Trump / meet / New York" stories into one Event. Thresholds
  (`OVERLAP_THRESHOLD`, `MIN_SHARED_WORDS`, `MIN_JACCARD`, `SHORT_HEADLINE_WORDS`) were tuned against that single
  snapshot, so re-check them on a few more pulls. Phase 3's LLM pass (same-event judgment) is the real fix; also
  worth trying description text or a lightweight stem/synonym step if that slips (description text was tried
  once and was noisier, not better). Merz's state-election defeat likewise splits into two Events.
- **Country resolution misses stories that name a leader/org but no country** — "Trump administration readies
  sanctions against ICC", "Exit polls show 'disaster' for Merz' CDU" are dropped `no-country` (336 of 1,020
  articles in the same pull, though many are genuinely off-topic — sport, culture). A leader→country and
  org→country table (or the LLM pass) would recover them. Also unresolved: Georgia (US state vs country) and
  Jordan/Chad-type ambiguities, which can false-positive.
...
- **Event `title` is an outlet's own headline, and `systemicThemes` is always empty** — both are Phase 3
  (LLM: neutral rewrite; theme assignment). `eventTimestamp` is the earliest report's publish time, not the
  moment of the occurrence.
- **The Phase 2 keyword classifier under-tiers.** It's a stand-in for the LLM pass (`src/news/classify.ts`):
  in the 2026-09-20 pull, 12 of 13 published Events were Significant and none Routine, and it has no
  "notable escalation" or "significant territorial change" beyond two regexes. Expected; don't tune it —
  Phase 3 replaces it behind the same `Classification` interface.
