...
- 42 Event(s) published from 1171 article(s) across 24/24 feeds; 0 pending confirmation.
- 405 article(s) sat in an Event below its corroboration floor (a lone report never publishes — every tier needs 2+ distinct sources) and were not emitted.
- 106 article(s) dropped for no resolvable country link (person names like "Trump"/"Merz" and orgs like the ICC don't resolve — see the standing item above) (first 25):
  - "Manchester United’s attack may be their best form of defence this season" (telegraph)
  ...
  - "Dangote Refinery braced for IPO" (semafor)
  ...
  - "AI agents hate CAPTCHA, too" (semafor)
...
- **Critical's 4-outlet fallback still counts sources, not independent newsgathering** (amended 2026-09-20
  because no wire feed — Reuters/AP/AFP — is reachable; state-controlled outlets are already excluded from the
  four, see `LOGBOOK.md`). Four outlets running one syndicated AP/Reuters story still pass. Fixing that needs
...
- **Embedding grouping: known weaknesses.** (1) ~10% of two-outlet Events on the live pull joined thematically related but different
  ...
  Venezuela deals) but recovered >=4-outlet stories 11/13 vs 8/13. (3) Tuned and scored on ONE pull with
...
- **Critical mostly does not fire at threshold 0.70 (J's choice).** The four-outlet fallback needs 4 outlets in ONE cluster and 0.70
  recovers 8/13 such stories (0.65: 11/13); on the 2026-09-21 pull the Riyadh attack split 3+3. Revisit if Critical staying dark
  becomes the bigger problem than the one false merge 0.65 accepts.
- **Classifier (relevance + tags): what's proven, what isn't.** Cross-validated on hand labels it beats the keyword rules on tags (macro-F1
  0.74 vs 0.60) and relevance (F1 0.86 vs 0.73), but: (1) the labels are Claude's, headline-only, one pull, several debatable (disasters:
  relevance depends on magnitude, which embeddings can't see); (2) there is NO clean held-out set — the "fresh" pull overlapped ~60% with
  the labeled one; (3) as an Event gate it is blunt (mild gate 0.30 drops only ~2 of 7 out-of-scope clusters; see LOGBOOK). Most useful
  next step: label the NEXT few days' pulls (~500 articles, plus cluster-level in-scope/out) as a real hold-out, then retrain on the union
  (`npm run train:news-classifier`, `npm run eval:news-classifier`). Each pull labeled is also more training data for the rare tags
  (terrorism 23, crime 19, humanitarian 29 examples) where F1 is still 0.44-0.67.
...
- **The eval fixtures are third-party text.** ~320 KB (clustering) + ~50 KB (labels) of RSS headlines/summaries under `scripts/fixtures/`,
  kept as evaluation/training input only. Decide whether that belongs in the repo long-term.
...
- **Feeds found unreachable or unusable, not ingested** (probed 2026-09-20): Kyodo (404), Eurasianet (404),
  ...
  the country-native roster were not probed — Critical's 4-outlet floor is only as reachable as the number of distinct
