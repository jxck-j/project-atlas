...
**Generated by `npm run build:news:events` (`scripts/buildNewsEvents.mjs`), 2026-09-21.** Re-running regenerates this list — don't hand-edit it.

- 42 Event(s) published from 1175 article(s) across 24/24 feeds; 0 pending confirmation.
- 421 article(s) sat in an Event below its corroboration floor (a lone report never publishes — every tier needs 2+ distinct sources) and were not emitted.
- 84 article(s) dropped for no resolvable country link (person names like "Trump"/"Merz" and orgs like the ICC don't resolve — see the standing item above) (first 25):
  ...
  - "Another New Job for Saida Mirziyoyeva: Manager of the Tashkent International Financial Center" (the-diplomat)
