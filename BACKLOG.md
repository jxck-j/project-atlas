...
- 244 Event(s) published from 5103 article(s) across 103/106 feeds; 0 pending confirmation.
- 1615 article(s) sat in an Event below its corroboration floor (a lone report never publishes — every tier needs 2+ distinct sources) and were not emitted.
- 3 feed(s) failed to fetch this run:
  - china-digital-times (https://chinadigitaltimes.net/feed/): 403 Forbidden
  - lana-tripoli (https://lana.gov.ly/rss/en/): fetch failed
  - daily-star-bangladesh (https://www.thedailystar.net/news/rss.xml): The operation was aborted due to timeout
- 157 article(s) dropped for no resolvable country link (person names like "Trump"/"Merz" and orgs like the ICC don't resolve — see the standing item above) (first 25):
  ...
  - "Kazakh oil industry struggling amid global energy crisis" (eurasianet)
  - "Europe launches two climate observation satellites" (deutsche-welle)
  - "Serbian Students Stage ‘Victors’ March’ to Register for Polls" (balkan-insight)
...
- **Full-roster ingestion follow-ups (opened 2026-09-23, when feeds went from 24 to 106 — see LOGBOOK):**
  - **37 roster profiles have no buildable feed**, each recorded with a reason in `src/news/feedGaps.json` (the
    coverage test keeps that file honest). Re-probe them periodically; the `checked` date on each says when.
    Worth a direct look: Reuters/AP (no public RSS at all), ISW and ACLED (the two specialist-verified orgs besides
    Bellingcat, neither has a feed; ACLED has an authenticated API), and the whole Sudan/Ethiopia/Egypt/Libya-east
    set (403 or unreachable), which leaves those pairings one-sided.
  - **Browser User-Agent: deferred by J (2026-09-23).** World Politics Review, Haaretz and CBC refuse the build's
    `ProjectAtlasNewsBot` UA and answer a browser one. They stay in `feedGaps.json` for now. If revisited, the
    narrow option is a per-feed UA override in `feeds.json` for just these three. A global browser UA gains
    nothing more (the other blocked sites refuse a browser UA too), hides the bot from every site, and risks IP
    blocks from detection that checks more than the UA string.
  - **Non-English feeds (31 sources) are archived, not built from** (`BUILD_LANGUAGES` in `eventBuilder.ts`). Options
    for using them: a multilingual embedding model plus translated country aliases, or translating title+description
    at build time. Either needs its own eval on labeled non-English samples before it can publish anything.
  - **Roster re-vetting surfaced by the probe:** The Daily Star (Lebanon) suspended publication in 2020; Al-Masdar
    Online's domain is parked; National Review "(News)" has no news-only feed; Tasnim's English edition appears gone
    (its `/en/` feeds serve Persian).
  - **Analysis orgs now corroborate** (§17b). Two think-tank pieces on one event clear Routine/Significant like two
    outlets would, and a lone Bellingcat piece publishes at Major or below. Both follow the design as written. Worth
    revisiting only if the published feed shows think-tank commentary propping up Events.
  - **Feed flakiness:** China Digital Times (403), LANA Tripoli and Daily Star BD each failed on one run after
    succeeding on the previous one. Watch the per-run failure list once runs are unattended (Phase 6).
