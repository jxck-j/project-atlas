...
- **Intelligence Engine** — as of v6.3.1, MILITARY is wired to real data
  ...
  **CURRENT STATUS is a different shape of gap, as of `scripts/
  ...
  independent fields, not a composite score." **What's still not done:**
  `AnalyticsPanel.tsx` — CURRENT STATUS is still hardcoded "Awaiting data
  feed" there, same as Diplomacy/Technology, since its ranked-list/
  sortable-column machinery doesn't obviously apply to this shape of data
  either (no single number to rank by); whatever its thumbnail ends up doing
  (a filtered list of currently-conflicted/sanctioned countries, most
  likely) needs its own design pass, not a `buildXRows`/`X_COLUMNS` copy.
  **Also open:** ORANGE/YELLOW sanction-tier assignments and program names
