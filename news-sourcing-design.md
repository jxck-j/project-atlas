...
**Severity calls settled from the labeled-data review (J, 2026-09-21).** Four cases the table above left open, decided against real headlines:
1. **A striking state's own casualty claim** ("Pakistan says 28 killed in airstrikes on three targets") is **Significant** until independently confirmed. Once confirmed, the ordinary single-incident rule applies (10+ deaths = Critical). A claim by a party does not count as a reported incident until someone other than that party reports it.
2. **Attack on a capital is Critical without confirmed casualties** (the table's "capital-city/embassy attack" row, confirmed — a missile at Riyadh with smoke near the airport). An attack on **infrastructure** (a pipeline, a fuel depot) with casualties below the threshold is **Major**.
3. **Market and macro records with no event behind them** ("10-year Treasury yield tops 5%", "UK borrowing costs hit a 19-year high") are **Significant** — "notable but non-shock". Major needs a shock (war, default, sanctions) or a real central-bank action.
4. **Legal follow-ups are not a new casualty.** A verdict against an ex-head-of-state or ex-leader (a war-crimes sentence) is **Major** — that is the ceiling for this shape, not a floor. Arrests, extraditions and trials over a *past* killing are **Significant**. The Critical "head-of-state casualty" trigger is for a new event only. **Generalized 2026-09-23 (J) from head-of-state killings to ANY past attack:** a judicial follow-up must not inherit the original attack's casualty figures at all, so the mass-casualty Critical and contained-strike Major triggers are both suppressed for it ("Sri Lanka court convicts 15 over 2019 Easter bombings" was reaching Critical off the 2019 attack's 270 deaths). Two boundaries this rule needs: it keys on *judicial* language only (a verdict takes months; "arrested"/"suspects" happen the same day as a fresh attack and must not downgrade it), and it floors at Significant rather than Routine (trial coverage is full of rhetoric words like "8 accused", which would otherwise sink it once it can no longer reach Critical).
5. **A contained strike in an ongoing war is Major only at 5+ deaths in the single incident**, or for a notable escalation (new weapon type, a previously untouched territory) regardless of count — not every strike with any casualties, which the table's literal wording would make Major. 1-4 deaths with no escalation marker is Significant; 10+ still follows the Critical single-incident rule (settled call 1 above governs whose deaths count, and whether the count is confirmed).
6. **A diplomatic rupture is split by act.** Formally severing relations is **Major**. Expelling or recalling an ambassador (without severing relations) is **Significant**.
7. **A capital attack is Critical only when it is a first-time or otherwise rare strike on that capital** (found 2026-09-21 while relabeling: applied literally, call 2 would make every Ukraine drone attack on Moscow Critical, since it's one of the most frequent headline shapes in that war — all 11 labeled examples had already, deliberately, been called Major). A ROUTINE/RECURRING capital strike within an already-active conflict — no casualty or escalation bar required, since it's already a real strike — is **Major**, the same "ongoing war has its own baseline" idea as call 5's contained-strike exception. An embassy attack keeps NO such carve-out (inherently rare even in an active war). The mechanical signal is a rarity/novelty phrase in the text ("for the first time", "unprecedented", "since the ... conflict resumed") — Riyadh's real headline said exactly this; Moscow's never did.
...
- **World tab (default landing):** ~~ranked by `linkedEntityIds.length` first — i.e. **trending across the most countries simultaneously** — then severity/recency within that. Chosen over "most severe worldwide" specifically to avoid the default view being dominated by whatever's most volatile that day regardless of breadth.~~ **Amended 2026-09-23 (J), Phase 4, after seeing it built:** breadth-first put Significant stories in the featured row while Critical and Major ones sat in the grid below it, which reads as a broken feed. Severity leads on World exactly as everywhere else; **breadth is now the tie-break *within* a severity tier**, so "trending across the most countries" still decides between two equally severe stories but can never outrank a more serious one.
- **Below the featured 3, everywhere:** ~~pure recency, no severity weighting~~ — **amended 2026-09-23 (J), Phase 4:** severity tier first, recency second, the same as everywhere else. v1 shipped the pure-recency rule and it was reported directly — a just-in Routine item sitting above an older Significant one reads as a broken feed. The amendment makes this section's own opening sentence ("same underlying rule everywhere") literally true; the featured 3 still differ only in *scope* (World ranks by breadth first), not in rule.
...
12. Admin/ops UI — **resolved and built** (Phase 5, 2026-09-23): manual data-entry only, houses the head-of-state-death review queue alongside source-leaning/theme-status editing (§14). Access control: none, loopback-only. Stack: its own React/Vite app. Calibration Review (§15f) remains unbuilt, pending the video pipeline.
...
**Status: BUILT (Phase 5, 2026-09-23, v6.13.0).** `npm run admin` — `admin/`, a separate Vite app on 127.0.0.1:5175 with dev-server middleware that reads and writes `sources.json`, `systemicThemes.json` and the review store directly. Both previously-open questions are resolved:

- **Access control: none.** It binds to loopback and the middleware independently refuses a non-loopback request. Auth would be guarding a developer's own files from themselves.
- **Stack: React/Vite**, as anticipated — but its OWN Vite config, not a route in the globe app, so `npm run build` never emits private tooling into `dist/`.

Two of the three scoped jobs shipped: editorial data entry (source roster + theme review) and the head-of-state-death queue. **Calibration Review (§15f) is not built** — it reviews video clips, so it waits on the first-hand/video pipeline existing.

One thing the build surfaced that this section didn't anticipate: §8's `manuallyConfirmed` could not have worked as written, because the build is stateless and re-runs the gate from scratch each time — a confirmation would have published once and then silently un-published itself. Decisions are now persisted outside the repo and re-attach by article URL as well as Event id. See `LOGBOOK.md`'s Phase 5 entry.
